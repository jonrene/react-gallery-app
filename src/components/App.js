...
const App = () => {
  ...
  useEffect(()=>{
    setcurrentQuery("pancakes%2Cwaffles%2Csyrup");
  },[])

  // Updates photo state of application when user enters a new search or clicks one of navigation search buttons
  useEffect(()=>{
    fetch(`https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=${apiInfo.key}&tags=${currentQuery}&per_page=24&format=json&nojsoncallback=1&safe_search=3`)
